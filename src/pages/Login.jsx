...
export default function Login(){
  ...
  return <><SEO title="Log In" description="Log in to your American Business Formations account." path="/login" noindex /><section className="auth-page"><div className="auth-shell"><div className="auth-side"><div><span>American Business Formations</span><h1>Your business journey, organized.</h1><p>Sign in to review progress, documents, deadlines, service requests, and recommendations.</p></div><img src="/illustrations/dashboard-preview.svg" alt="Dashboard preview"/></div><div className="auth-form-wrap"><Link className="auth-back" to="/">← Back to website</Link><form className="auth-form" onSubmit={submit} noValidate><span>Welcome back</span><h2>Log in to your account</h2><p>Use any valid email and password in the demo.</p>
    ...
    <label className="check-control terms-check"><input type="checkbox" name="is_admin"/> Sign in as demo admin (for testing the admin portal only)</label><div className="form-between"><label className="check-control"><input type="checkbox"/> Remember me</label><Link to="/forgot-password">Forgot password?</Link></div><button className="btn btn-primary btn-block" disabled={loading}>{loading&&<Loader2 className="spin" size={18}/>}{loading?'Signing in...':'Log in'}</button><p className="auth-switch">New to ABF? <Link to="/signup">Create an account</Link></p></form></div></div></section></>
}
