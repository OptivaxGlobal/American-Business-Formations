...
export default function Businesses(){
  const {businesses,loading,error,refetch,selectedBusinessId,setSelectedBusinessId}=useBusiness()
  return <div className="dash-card">
    ...
    <AsyncState loading={loading} error={error} onRetry={refetch} loadingLabel="Loading your businesses…">
      ...
      <div className="business-list">
        {businesses.map((b,i)=><Reveal as="div" delay={i%6} key={b.id} className={`business-list-item ${b.id===selectedBusinessId?'active':''}`}>
          ...
          <span><strong>{b.name}</strong><small>{b.entity_type} • {b.state} • {b.status}</small></span>
