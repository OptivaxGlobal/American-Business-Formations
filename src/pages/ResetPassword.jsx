...
export default function ResetPassword(){
  ...
  return <><SEO title="Reset Password" description="Choose a new password for your account." path="/reset-password" noindex /><section className="auth-page"><div className="auth-shell"><div className="auth-side"><div><span>Account recovery</span><h1>Choose a new password.</h1><p>Use at least 8 characters, including uppercase, lowercase, a number, and a special character.</p></div><img src="/illustrations/hero-business.svg" alt="Business illustration"/></div><div className="auth-form-wrap"><Link className="auth-back" to="/login">← Back to log in</Link><form className="auth-form" onSubmit={submit} noValidate><span>Reset password</span><h2>Create a new password</h2>
    ...
    <button className="btn btn-primary btn-block" disabled={loading}>{loading?'Updating...':'Update password'} <ArrowRight size={18}/></button></form></div></div></section></>
}
